...
- **exe 桌面版（`clipboard-exe/`，M1+M2+M3a+M3b-1+M3b-2a+M3b-2a-1 已落地，M3b-2b/M3b-3/M4/M5 进行中）**：C# WPF net9.0-windows 原生重写（非壳、非旧 WinForms 路线——旧路线已废弃不参考）。**还原铁律**：规则类逻辑（sortClips / rollToArchive / isExpired / resolveExpire / cleanUrl / mergeSnapshots / 墓碑 / 文件存储 files.js）逐字搬运与 Web 版一致，**唯一新增功能 = 置顶按钮**（Window.Topmost）；数据 JSON 与 Web 版互导。数据层文件：`Models/ClipItem.cs`（17 字段 camelCase）+ `Services/Storage.cs`（原子写/排序/归档）+ `Services/FileStore.cs`（文件实体 save/getPath/delete，对齐 files.js）+ `Services/Pinyin.cs`（3755 字）+ `Services/CleanUrl.cs` + `Services/ClipService.cs` + `Services/ClipboardWatcher.cs`（WM_CLIPBOARDUPDATE + 防抖 + 来源抑制 + 前台语义）；UI 文件：`Controls/PasteDialog.xaml(.cs)`（文本/链接/文件 chip + 粘贴/拖放/10MB 拒收 + 图片 2a 拒收 toast）+ `Controls/CardView.xaml(.cs)`（文本/链接/文件图标卡 PDF 红边 ZIP 金边）+ `Controls/EditDialog.xaml(.cs)` + `Controls/JsonDialog.xaml(.cs)` + `Controls/TagPicker.xaml(.cs)`；UI 规范：`Themes/Colors.xaml` 圆角令牌（RadiusFold/RadiusIcon/RadiusIconSm/RadiusIconLg/RadiusBtnSm/RadiusBtn/RadiusMd/RadiusLg/RadiusPill 全令牌化）+ `Themes/Styles.xaml`（含全局 ToolTip 覆盖系统默认矩形）+ `Controls/NeuBorder.cs` + `Controls/PillBorder.cs`；健康检查 `--selftest`（**126 断言**：M2 42 + M3a 43 + M3b-1 14 + M3b-2a 27，M3b-2a-1 修复无新增断言零回归）。方案/模块映射/验收门禁见 `docs/exe-wpf-plan.md`。改 Web 版排序/合并/清理/文件存储规则时必须同步改 clipboard-exe 对应实现，反之亦然（双版本规则一致是准入门槛）
