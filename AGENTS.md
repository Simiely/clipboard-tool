...
- **exe 桌面版（`clipboard-exe/`，M1+M2+M3a+M3b-1+M3b-2a+M3b-2a-1+M3b-2b 已落地，M3b-3/M4/M5 进行中）**：C# WPF net9.0-windows 原生重写（非壳、非旧 WinForms 路线——旧路线已废弃不参考）。**还原铁律**：规则类逻辑（sortClips / rollToArchive / isExpired / resolveExpire / cleanUrl / mergeSnapshots / 墓碑 / 文件存储 files.js）逐字搬运与 Web 版一致，**唯一新增功能 = 置顶按钮**（Window.Topmost）；数据 JSON 与 Web 版互导。数据层文件：`Models/ClipItem.cs`（17 字段 camelCase）+ `Services/Storage.cs`（原子写/排序/归档）+ `Services/FileStore.cs`（文件实体 save/getPath/delete，对齐 files.js）+ `Services/Format.cs`（新增 `IsImageMime` 统一图片判定入口）+ `Services/Pinyin.cs`（3755 字）+ `Services/CleanUrl.cs` + `Services/ClipService.cs` + `Services/ClipboardWatcher.cs`（WM_CLIPBOARDUPDATE + 防抖 + 来源抑制 + 前台语义）；UI 文件：`Controls/PasteDialog.xaml(.cs)`（文本/链接/文件/图片 chip + 粘贴/拖放/10MB 拒收 + Bitmap 截图 EncodePng 接收）+ `Controls/CardView.xaml(.cs)`（文本/链接/文件图标卡 PDF 红边 ZIP 金边 + 图片卡体 UniformToFill cover + hover 悬浮预览 260ms/50%~300% Popup 视口钳制 + 复制图片 CopyImageRequested）+ `Controls/EditDialog.xaml(.cs)` + `Controls/JsonDialog.xaml(.cs)` + `Controls/TagPicker.xaml(.cs)`；UI 规范：`Themes/Colors.xaml` 圆角令牌（RadiusFold/RadiusIcon/RadiusIconSm/RadiusIconLg/RadiusBtnSm/RadiusBtn/RadiusMd/RadiusLg/RadiusPill 全令牌化）+ 紫色 `PurpleBrush`/`PurpleSoftBrush`（图片徽章）+ `Themes/Styles.xaml`（含全局 ToolTip 覆盖系统默认矩形 + `BadgeImage` 紫色样式）+ `Controls/NeuBorder.cs` + `Controls/PillBorder.cs`；健康检查 `--selftest`（**144 断言**：M2 42 + M3a 43 + M3b-1 14 + M3b-2a 27 + M3b-2b 18）。方案/模块映射/验收门禁见 `docs/exe-wpf-plan.md`。改 Web 版排序/合并/清理/文件存储/图片规则时必须同步改 clipboard-exe 对应实现，反之亦然（双版本规则一致是准入门槛）

## 关键坑（3~5 条，越具体越好）
- **exe 版 GUI 程序验证：bash 启动不等待、无 stdout（M2 实战坑）**：`Clipboard.exe` 是 WinExe——bash/cmd 直跑**立即返回**（`$?` 是假象），`Console.WriteLine` 也看不见。验证退出码必须 `python subprocess.run([exe, "--selftest"])`（等待 + 真实 returncode）或 PowerShell `Start-Process -Wait`；自检明细看运行目录 `selftest.log`。**构建输出在 `bin/Release/net9.0-windows/Clipboard.exe`**（csproj 无 RuntimeIdentifier），`win-x64/` 与 `publish/` 是旧发布产物——别误跑旧 exe 得出"代码没生效"结论
- **C# `List.Sort` 不稳定、JS `sort` 稳定（M2 迁移坑）**：Web 版排序依赖稳定排序保持同级顺序，C# 必须用 LINQ `OrderBy/ThenBy/ThenBy`（稳定链），不能 `List.Sort` 多级比较；`Storage.SortClips` 是唯一实现，自检含排序对拍
- **ClipItem JSON 序列化（M2）**：字段用显式 `JsonPropertyName`（camelCase，不依赖全局 NamingPolicy），17 字段与 Web `publicClip` 完全一致；字段名 `expireAt`（非 `expiresAt`）、`html` 非 `richHtml`、`fileMime` 非 `mime`——互导校验按此清单，别按旧规划文档（已修正）
