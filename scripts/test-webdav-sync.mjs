...
const snap = await davGet("workbuddy/剪贴板/clipboard-WebDAV测试.json");
...
const snap2 = await davGet("workbuddy/剪贴板/clipboard-WebDAV测试.json");
...
const fRemote = await fetch(DAV + "workbuddy/剪贴板/files/WebDAV测试/" + up2.file.fileId + ".png", { headers: AUTH });
...
await req("POST", "/api/sync/run", { token: tk });
const fNoRemote = await fetch(DAV + "workbuddy/剪贴板/files/WebDAV测试/" + up3.file.fileId + ".txt", { headers: AUTH });
ok("不勾选实体:远端无文件实体", fNoRemote.status === 404);
// 但快照里仍有 file 条目元数据
const snap3 = await davGet("workbuddy/剪贴板/clipboard-WebDAV测试.json");
