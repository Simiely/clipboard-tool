...
| ⏳ **M5b 同步引擎**（待续） | `WebDavSync.runSync`(collectRemoteSources/mergeRemoteAll/writeBackLocal/cleanupMigrations) + 文件实体双向同步 `syncFileEntities` | 一键同步闭环（无头环境改契约自检） |
| ⏳ **M5c 配置 UI + 定时同步**（待续） | `DataDialog` WebDAV 配置页（测试保存/一键同步/开关/间隔）+ 后台定时器按 intervalMin 自动同步 | 配置 UI 渲染 + 定时器触发 runSync |
