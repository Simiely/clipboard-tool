// Services/Pinyin.cs - 汉字拼音首字母映射（对齐 Web 版 public/app.js PY_GROUPS + strToPy）
// PY_GROUPS 为 GB2312 一级字库 3755 字，按拼音首字母分 23 组（I/U/V 无字，与 A-Z 其余字母一一对应）。
// 数据逐字搬运自 app.js L723（python 脚本提取 + 双校验：总数 3755、无重复字），勿手改。
using System.Text;
...
{
    /// <summary>字库分组数（自检断言：23）。</summary>
    public const int GroupCount = 23;

    /// <summary>字库总字数（GB2312 一级字库；自检断言：3755）。</summary>
    public const int TotalCharCount = 3755;

    /// <summary>实际装载字数（== TotalCharCount，双保险）。</summary>
    public static int TotalChars => Map.Count;

    private static readonly Dictionary<char, char> Map = BuildMap();

    private static Dictionary<char, char> BuildMap()
    {
        var map = new Dictionary<char, char>(TotalCharCount);
        var groups = new (char Letter, string Chars)[]
        {
            ('A', "啊阿埃挨哎唉哀皑癌蔼矮艾碍爱隘鞍氨安俺按暗岸胺案肮昂盎凹敖熬翱袄傲奥懊澳"),
            ('B', "芭捌扒叭吧笆八疤巴拔跋靶把耙坝霸罢爸白柏百摆佰败拜稗斑班搬扳般颁板版扮拌伴瓣半办绊邦帮梆榜膀绑棒磅蚌镑傍谤苞胞包褒剥薄雹保堡饱宝抱报暴豹鲍爆杯碑悲卑北辈背贝钡倍狈备惫焙被奔苯本笨崩绷甭泵蹦迸逼鼻比鄙笔彼碧蓖蔽毕毙毖币庇痹闭敝弊必壁臂避陛鞭边编贬扁便变卞辨辩辫遍标彪膘表鳖憋别瘪彬斌濒滨宾摈兵冰柄丙秉饼炳病并玻菠播拨钵波博勃搏铂箔伯帛舶脖膊渤驳捕卜哺补埠不布步簿部怖"),
            ...
            ('O', "哦欧鸥殴藕呕偶沤"),
            ('P', "辟泊脯啪趴爬帕怕琶拍排牌徘湃派攀潘盘磐盼畔判叛乓庞旁耪胖抛咆刨炮袍跑泡呸胚培裴赔陪配佩沛喷盆砰抨烹澎彭蓬棚硼篷膨朋鹏捧碰坯砒霹批披劈琵毗啤脾疲皮匹痞僻屁譬篇偏片骗飘漂瓢票撇瞥拼频贫品聘乒坪苹萍平凭瓶评屏坡泼颇婆破魄迫粕剖扑铺仆莆葡菩蒲埔朴圃普浦谱曝瀑"),
        ...
        foreach (var (letter, chars) in groups)
            foreach (var ch in chars)
                map[ch] = letter;
        return map;
    }

    /// <summary>取单字拼音首字母；非字库字符返回空串（对齐 pyInitial）。</summary>
    public static string InitialOf(char ch)
    {
        return Map.TryGetValue(ch, out var letter) ? letter.ToString() : "";
    }

    /// <summary>整串转拼音首字母缩写（对齐 strToPy：逐字取首字母，非汉字跳过）。</summary>
    public static string InitialsOf(string? s)
    {
        if (string.IsNullOrEmpty(s)) return "";
        var sb = new StringBuilder(s.Length);
        foreach (var ch in s) sb.Append(InitialOf(ch));
        return sb.ToString();
    }
}
