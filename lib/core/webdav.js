...
export async function runSync(userId) {
  ...
  try {
    ...
    // 账号名修改迁移源（可能多个——连续改名的旧名逐个迁移；P-1：删除成功才移除，失败保留下次重试）
    const pendingNames = (cfg.pendingNameMigrations || []).filter((n) => n && n !== name);
    for (const pn of pendingNames) {
      const pu = snapUrl(cfg, pn);
      const ps = await fetchRemoteSnapshot(cfg, pu);
      if (ps) srcs.push([pu, ps]);
    }
    ...
    if (cfg.syncFiles) await syncFileEntities(cfg, name, userId, merged.clips, pendingNames);
