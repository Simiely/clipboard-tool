...
export function saveSyncConfig(userId, { url, user, pass, syncFiles, autoSync, intervalMin }) {
  ...
  // v0.6.19：syncFiles 默认 true（此前默认 false —— 绝大多数用户根本不知道有这个开关，
 ...
 *
 * v0.6.19 语义修正（关键）：`syncFiles` 只控制「是否备份实体上云」，**不再控制拉回**。
...
export async function runSync(userId) {
  ...
  try {
    ...
    // ④ 实体同步（v0.6.19 无条件执行）：本地有 → 当 syncFiles 勾选时上传到 files/<accountName>/；
...
// ---------- v0.6.19 一次性迁移：存量 syncFiles=false → true ----------

/**
 * 把存量配置里 `syncFiles:false` 翻成 true（仅一次，用 `v0619SyncFiles` 标记防重复）。
 *
 * 背景：v0.6.19 之前 `saveSyncConfig` 恒写 `syncFiles: !!syncFiles`，未勾选就是 false，
 * 而前端复选框默认不勾 → **几乎每个用户的配置里都是 false，且并非其主动选择**。
 * 后果是文件/图片实体从未备份上云，换设备同步后条目可见但实体缺失、下载 404。
 * v0.6.19 已把新配置默认改为 true，但存量数据不会自动变——本函数补上这一刀。
 *
...
export function migrateSyncFilesDefaults() {
  ...
  try {
    ...
    for (const f of fs.readdirSync(CONFIG.usersDir)) {
      ...
      if (!cfg || typeof cfg !== "object" || cfg.v0619SyncFiles) continue;
      cfg.v0619SyncFiles = true;
