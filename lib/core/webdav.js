...
//   8) 按用户名寻址（v0.6.13）：快照 = clipboard-<用户名>.json、实体 = files/<用户名>/——设备迁移零配置：
//      新部署创建同名用户 → 配置 WebDAV → 同步即拉回全部数据（用户名即身份）。改名自动迁移：
//      同步配置记 syncedName，发现旧名快照/旧格式 clipboard-<userId>.json 存在 → 并入合并后上传当前名并清理旧路径。
//      同部署内用户名唯一（409 校验），不同部署同名=同一数据（单机自用场景即"迁移"语义）
...
export async function runSync(userId) {
  ...
  try {
    ...
    const name = getUserName(userId); // v0.6.13 按用户名寻址：快照 = clipboard-<用户名>.json
    await ensureDir(cfg);

    // 远端数据源 = 当前名快照 + 迁移源（旧格式 clipboard-<userId>.json 兼容 + 改名前的旧名快照）
    // 迁移源存在 → 并入合并（updatedAt 取新），上传到当前名后清理旧路径——设备迁移/改名数据不丢
    const curUrl = snapUrl(cfg, name);
    const legacyUrl = snapUrl(cfg, userId); // 旧格式（v0.6.13 前按 userId 存）
    const oldNameUrl = cfg.syncedName && cfg.syncedName !== name ? snapUrl(cfg, cfg.syncedName) : null;
    ...
    if (oldNameUrl && oldNameUrl !== curUrl) {
      const oldSnap = await fetchRemoteSnapshot(cfg, oldNameUrl);
      if (oldSnap) srcs.push([oldNameUrl, oldSnap]);
    }
    ...
    // 实体同步（勾选时）：本地有 → 上传到 files/<name>/；本地缺（恢复）→ 从 files/<name>/ 拉回
    ...
    // 迁移清理：合并上传成功后删除旧路径快照（数据已并入当前名）。删除失败 → 不推进 syncedName，
    // 下次同步继续检测迁移（幂等自愈）。legacy(userId 格式)删除失败无碍（下次 404 跳过）。
    let migrateOk = true;
    for (const [url] of srcs) {
      if (url === curUrl) continue;
      const dr = await davFetch(url, { method: "DELETE", headers: { Authorization: authHeader(cfg) } }).catch(() => ({ status: 0 }));
      if (![200, 204, 404].includes(dr.status)) migrateOk = false;
    }

    updateConfig(userId, {
      lastSyncAt: Date.now(), lastSyncError: "", // 成功即清失败标记（P-104）
      syncedName: migrateOk ? name : (cfg.syncedName || name), // 记录已同步名（改名迁移基准；删除失败保持旧值以便下次重迁）
    });
    ...
    return {
      ok: true,
      remoteExisted,
      migrated, // v0.6.13：本次是否发生过旧数据迁移（旧格式/改名路径）
      uploaded: hadLocal,
