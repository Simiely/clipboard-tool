...
/** v0.6.13 双名模型·账号名修改后调用：记 prevAccountName，下次同步自动把旧账号名快照并入并迁移（管理员级一次性操作，幂等自愈） */
export function noteAccountRename(userId, oldName) {
  const cfg = getSyncConfig(userId);
  if (!cfg) return;
  writeJson(cfgFile(userId), { ...cfg, prevAccountName: oldName });
}
...
export async function runSync(userId) {
  ...
  try {
    ...
    // 远端数据源 = 当前账号名快照 + 旧格式 clipboard-<userId>.json（v0.6.13 前数据）+ 账号名修改前的旧名快照
    ...
    const prevName = cfg.prevAccountName; // v0.6.13 账号名修改迁移（一次性：推进后不再检测）
    if (prevName && prevName !== name) {
      const prevSnap = await fetchRemoteSnapshot(cfg, snapUrl(cfg, prevName));
      if (prevSnap) srcs.push([snapUrl(cfg, prevName), prevSnap]);
    }
