...
/** 会话写入一致性（走查 P-A 修复）：文件 sessions.json 为最终权威（重启后从文件加载），
 *  内存变更必须与文件同生共死——persistSessions 失败即回滚内存，保证「内存 ⇔ 文件」原子一致：
 *  - createToken 写失败 → 回滚删除内存 token（重启不产生幽灵登录态）
 *  - destroyToken 写失败 → 回滚恢复内存 token（重启不复活已删 token）
 *  - verifyToken 惰性清理写失败 → 回滚恢复（同上） */
export function createToken(userId) {
  ensureSessionsLoaded();
  const token = crypto.randomBytes(CONFIG.TOKEN_BYTES).toString("hex");
  const now = Date.now();
  const s = { userId, createdAt: now, expireAt: now + SESSION_TTL_MS };
  sessions.set(token, s);
  try { persistSessions(); }
  catch (e) { sessions.delete(token); throw e; } // 文件失败 → 内存回滚，不产生不一致
  return token;
}
...
export function verifyToken(token) {
  ...
  if (s.expireAt && s.expireAt <= Date.now()) { // 过期即删（惰性清理）
    ...
    try { persistSessions(); }
    catch (e) { sessions.set(token, s); throw e; } // 文件失败 → 回滚恢复
    return null;
  }
  return s.userId;
}
...
export function destroyToken(token) {
  ...
  sessions.delete(token);
  try { persistSessions(); }
  catch (e) { sessions.set(token, s); throw e; } // 文件失败 → 回滚恢复（重启不复活）
}
