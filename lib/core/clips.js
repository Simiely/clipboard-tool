 ...
 *  P-102 修复：墓碑不再在此无条件记录——改为返回 tombstone 数据，由路由层判断
 *  「已配置 WebDAV」才调用 recordTombstone（未配置同步的用户删除不产生墓碑文件）。
 ...
 *  与 deleteClip 同语义：返回 tombstone 供路由层判断「已配置 WebDAV」才记录（删除传播到远端）。 */
...
export function archiveClip(userId, id) {
  ...
  saveClips(userId, list);
  const arch = loadArchive(userId);
  if (!arch.some((c) => c.id === clipId)) { arch.push(moved); writeJson(archiveFile(userId), arch); }
  return { ok: true };
}
