...
- 🟡 待续：M5b 同步引擎（collectRemoteSources/runSync/writeBackLocal/cleanupMigrations + 文件实体同步）+ M5c 配置 UI + 定时自动同步
...
- 🟢 **平台版快捷键调整（2026-09-02，对齐 exe 桌面版）**：主页面快捷键由「空格=存入」改为「**Ctrl+V=存入** / **空格=聚焦搜索框**」。
