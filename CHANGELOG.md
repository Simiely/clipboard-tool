...
- 🟢 **WebDAV 按用户名寻址（v0.6.13 设备迁移零配置）**：快照 `clipboard-<用户名>.json`、实体 `files/<用户名>/`——新部署创建同名用户 → 配置 WebDAV → 同步即拉回全部数据（用户名即身份）。含两类自动迁移：①改名后同步自动并入旧名快照并删除旧路径（配置记 `syncedName` 基准，删除失败不推进下次重迁，幂等自愈）②旧格式 `clipboard-<userId>.json` 首次同步自动并入并迁移。`runSync` 返回 `migrated` 标记
